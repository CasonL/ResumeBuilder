...
export async function POST(request: NextRequest) {
  try {
    ...
    // Format as "Company - Role" so the dashboard card displays both prominently
    const aiName = (finalResult as any).resumeName || 'Generated Resume';
    const company = extractCompanyName(jobDescription);
    const resumeName = company ? `${company} - ${aiName}` : aiName;
